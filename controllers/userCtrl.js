...
const userCtrl = {
  ...
  deleteUserAdmin: async (req, res) => {
    try {
      ...
      await Posts.deleteMany({ user: { $in: req.params.id } });
      await Comments.deleteMany({ user: { $in: req.params.id } });

      if (res.status === 200) {
        console.log("first", posts, user);
      }
      res.json({
        msg: "Deleted User!",
      });
  ...
  updateUserAdmin: async (req, res) => {
    try {
      const { fullname, mobile, address, story, website, gender, role } =
        req.body;
      if (!fullname)
        return res.status(400).json({ msg: "Please add your full name." });
